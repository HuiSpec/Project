...
#include "GPS_Usart.h"
#include <stdio.h>
char USART2_RxPacket[100]; //数据暂存
uint8_t USART2_RxFlag;     //定义接收数据包标志位

char GNGGA_Packet[100]; //定义接收数据包数组GNGGA
char GNGLL_Packet[100]; //定义接收数据包数组GNGLL
char GNZDA_Packet[100]; //定义接收数据包数组GNZDA

void GPS_copy()
{
    if (strstr(USART2_RxPacket, "$GNGGA,"))
    {
        strcpy(GNGGA_Packet, USART2_RxPacket);
    }
    else if (strstr(USART2_RxPacket, "$GNGLL,"))
    {
        strcpy(GNGLL_Packet, USART2_RxPacket);
    }
    else if (strstr(USART2_RxPacket, "$GNZDA,"))
    {
        strcpy(GNZDA_Packet, USART2_RxPacket);
    }
}
...
void USART2_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(USART2_GPIO_CLK, ENABLE);       //开启GPIOA的时钟
    ...
    GPIO_Init(USART2_GPIO_PORT, &GPIO_InitStructure); //将PA9引脚初始化为复用推挽输出

    GPIO_InitStructure.GPIO_Mode  = GPIO_Mode_IPU;
    GPIO_InitStructure.GPIO_Pin   = USART2_RX_GPIO_PIN;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(USART2_GPIO_PORT, &GPIO_InitStructure); //将PA10引脚初始化为上拉输入
...
void USART2_SendNumber(uint32_t Number, uint8_t Length)
{
    ...
    {
        USART2_SendByte(Number / USART2_Pow(10, Length - i - 1) % 10 +
                        '0'); //依次调用USART2_SendByte发送每位数字
    }
}
...
void USART2_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET) //判断是否是USART2的接收事件触发的中断
    {
        ...
        // if (RxState == 0 & RxData == "@")
        // {
        //     if (USART2_RxFlag == 0) //如果数据确实是包头，并且上一个数据包已处理完毕
        //     {
        //         RxState = 1; //置下一个状态
        //         // pRxPacket = 0; //数据包的位置归零
        //         USART2_RxPacket[pRxPacket] = RxData; //将数据存入数据包数组的指定位置
        //         pRxPacket++;                         //数据包的位置自增
        //     }
        // }
        ...
        else if (RxState == 1)
        {
            ...
            {
                RxState                    = 0; //状态归0
                USART2_RxPacket[pRxPacket] = '\0'; //将收到的字符数据包添加一个字符串结束标志

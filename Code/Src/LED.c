...
// 此函数用于切换LED1的状态。如果当前LED1处于关闭状态（即PA1引脚输出低电平），则将其打开（设置PA1引脚为高电平）；
// 如果当前LED1处于打开状态（即PA1引脚输出高电平），则将其关闭（设置PA1引脚为低电平）。

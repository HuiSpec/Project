...
char USART1_RxPacket[100]; //定义接收数据包数组，数据包格式
char CSQ_Packet[10];
char IP_Packet[10];
char CIMI_Packet[100]; //定义接收数据包数组CIMI
char CEREG_Packet[10];
uint8_t USART1_RxFlag; //定义接收数据包标志位

void NBIOT_copy()
{
    if (strstr(USART1_RxPacket, "+CSQ:"))
    {
        strcpy(CSQ_Packet, USART1_RxPacket);
    }
    else if (USART1_RxPacket[0] <= '9' && USART1_RxPacket[0] >= '0')
    {
        strcpy(CIMI_Packet, USART1_RxPacket);
    }
    else if (strstr(USART1_RxPacket, "+CEREG:"))
    {
        strcpy(CEREG_Packet, USART1_RxPacket);
    }
    else if (strstr(USART1_RxPacket, "+IP"))
    {
        strcpy(IP_Packet, &USART1_RxPacket[1]);
    }
}
...
void USART1_SendNumber(uint32_t Number, uint8_t Length)
{
    ...
    {
        USART1_SendByte(Number / USART1_Pow(10, Length - i - 1) % 10 +
                        '0'); //依次调用USART1_SendByte发送每位数字
    }
}
...
void USART1_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(USART1, USART_IT_RXNE) == SET) //判断是否是USART1的接收事件触发的中断
    {
        ...
        else if (RxState == 1)
        {
            ...
            {
                RxState                    = 0; //状态归0
                USART1_RxPacket[pRxPacket] = '\0'; //将收到的字符数据包添加一个字符串结束标志
